...
def creat_log_dir(args):
    run_id = '{}-{}-{}-{}'.format(args.agent, args.task, args.seed, time.strftime('%Y-%m-%d-%H-%M-%S'))
    log_dir = os.path.join(args.logdir, args.task, args.agent, run_id)
...
def save_code(save_dir):
    ...
    shutil.copytree(project_dir, save_dir + '/code',
                    ignore=shutil.ignore_patterns('venv', 'log', 'result', 'refs', '.git', '*.pyc', '.idea', '.DS_Store',
                                                  'gym_minigrid'))
